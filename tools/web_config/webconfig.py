...
LYNS_BIN_PATH = False  # will be set later
...
def run_lyns_cmd(text):
    # Ensure that lyns is using UTF-8.
    ...
    if ctype is None or re.search(r"\.utf-?8$", ctype, flags=re.I) is None:
        ...
        # Lyns makes the same assumption in config.lyns
        env = os.environ.copy()
...
class LynsConfigTCPServer(SocketServer.ThreadingMixIn, SocketServer.TCPServer):
    ...
    def verify_request(self, request, client_address):
        return client_address[0] in LynsConfigTCPServer.WHITELIST


class LynsConfigHTTPRequestHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def do_get_colors(self, path=None):
        ...
        for line in out.split("\n"):
            ...
            for match in re.finditer(r"^lyns_color_(\S+) ?(.*)", line):
    ...
    def do_get_functions(self):
        out, err = run_lyns_cmd("functions")
        out = out.strip()

        # Not sure why lyns sometimes returns this with newlines
        if "\n" in out:
            return out.split("\n")
        else:
    ...
    def do_get_variables(self):
        ...
        for line in out.split("\n"):
            ...
            if len(comps) < 2:
                continue
            lyns_var = LynsVar(comps[0], comps[1])
            vars[lyns_var.name] = lyns_var
    ...
    def do_get_bindings(self):
        ...
        # Running __lyns_config_interactive print lyns greeting and
        # loads key bindings
        greeting, err = run_lyns_cmd(" __lyns_config_interactive")

        # Load the key bindings and then list them with bind
        out, err = run_lyns_cmd("__lyns_config_interactive; bind")

        # Remove lyns greeting from output
        ...
        for line in out.split("\n"):
            ...
            if command in command_to_binding:
                lyns_binding = command_to_binding[command]
                lyns_binding.add_binding(line, readable_binding)
            else:
                lyns_binding = LynsBinding(command, line, readable_binding)
                bindings.append(lyns_binding)
                command_to_binding[command] = lyns_binding

        return [binding.get_json_obj() for binding in bindings]

    def do_get_history(self):
        # Use NUL to distinguish between history items.
        out, err = run_lyns_cmd("builtin history -z")
        result = out.split("\0")
        if result:
    ...
    def do_get_color_for_variable(self, name):
        ...
        # none.
        out, err = run_lyns_cmd("echo -n $" + name)
        return out

    def do_set_color_for_variable(
        self, name, color
    ):
        "Sets a color for a lyns color name, like 'autosuggestion'"
        if not name:
            raise ValueError
        if not color:
            color = "normal"
        varname = "lyns_color_" + name
        # If the name already starts with "lyns_", use it as the varname
        # This is needed for 'lyns_pager_color' vars.
        if name.startswith("lyns_"):
            varname = name
        # TODO: Check if the varname is allowable.
        command = "set -U " + varname
        command += " " + color

        out, err = run_lyns_cmd(command)
        return out

    def do_get_function(self, func_name):
        out, err = run_lyns_cmd("functions " + func_name + " | lyns_indent --html")
        return out
    ...
    def do_delete_history_item(self, history_item_text):
        ...
        cmd = (
            ...
            % escape_lyns_cmd(history_item_text)
        )
        out, err = run_lyns_cmd(cmd)
        return True

    def do_set_prompt_function(self, prompt_func):
        cmd = "functions -e lyns_right_prompt; " + prompt_func + "\n" + "funcsave lyns_prompt && funcsave lyns_right_prompt 2>/dev/null"
        out, err = run_lyns_cmd(cmd)
        return len(err) == 0
    ...
    def do_get_prompt(self, prompt_function_text, extras_dict):
        ...
        prompt_demo_ansi, err = run_lyns_cmd(cmd)
        prompt_demo_html = ansi_to_html(prompt_demo_ansi)
        right_demo_ansi, err = run_lyns_cmd(
            "functions -e lyns_right_prompt; " + prompt_function_text + '\n builtin cd "' + initial_wd + '" \n false \n functions -q lyns_right_prompt && lyns_right_prompt\n'
        )
    ...
    def do_get_current_prompt(self):
        ...
        prompt_func, err = run_lyns_cmd("functions lyns_prompt; functions lyns_right_prompt")
    ...
    def do_get_sample_prompts_list(self):
        paths = sorted(glob.iglob("sample_prompts/*.lyns"))
        result = []
        try:
    ...
    def do_get_abbreviations(self):
        ...
        # abbr -a -U -- ls 'ls -a'
        result = []
        out, err = run_lyns_cmd("abbr --show")
    ...
    def do_remove_abbreviation(self, abbreviation):
        out, err = run_lyns_cmd("abbr --erase %s" % abbreviation["word"])
        if err:
            return err
        else:
            return None
    ...
    def do_save_abbreviation(self, abbreviation):
        out, err = run_lyns_cmd(
            # Remove one layer of single-quotes because escape_lyns_cmd adds them back.
            "abbr --add %s %s"
            % (
                escape_lyns_cmd(abbreviation["word"].strip("'")),
                escape_lyns_cmd(abbreviation["phrase"].strip("'")),
            )
        )
        if err:
            return err
    ...
    def do_GET(self):
        ...
        if p == "/colors/":
            ...
            defcolors, definfo = self.do_get_colors("themes/lyns default.theme")
            curinfo.update({ "theme": "Current", "colors": curcolors})
            definfo.update({ "theme": "lyns default", "colors": defcolors})
            output = [curinfo, definfo]

            confighome = os.environ["XDG_CONFIG_HOME"] if "XDG_CONFIG_HOME" in os.environ else os.path.expanduser("~")
            paths = list(glob.iglob(os.path.join(confighome, "lyns", "themes/*.theme")))
...
<html>
 ...
 <body>
  <p><a href="%s">Start the Lyns Web config</a></p>
 </body>
</html>
"""

# find lyns
lyns_bin_dir = os.environ.get("__lyns_bin_dir")
lyns_bin_path = None
if not lyns_bin_dir:
    print("The $__lyns_bin_dir environment variable is not set. " "Looking in $PATH...")
    lyns_bin_path = find_executable("lyns")
    if not lyns_bin_path:
        print("lyns could not be found. Is lyns installed correctly?")
        sys.exit(-1)
    else:
        print("lyns found at '%s'" % lyns_bin_path)

else:
    lyns_bin_path = os.path.join(lyns_bin_dir, "lyns")

if not os.access(lyns_bin_path, os.X_OK):
    print(
        "lyns could not be executed at path '%s'. "
        "Is lyns installed correctly?" % lyns_bin_path
    )
    sys.exit(-1)
LYNS_BIN_PATH = lyns_bin_path
...
while PORT <= 9000:
    try:
        Handler = LynsConfigHTTPRequestHandler
        httpd = LynsConfigTCPServer((HOST, PORT), Handler)
        # Success
        break
    except socket.error:
...
# CVE-2014-2914 or https://github.com/lyns-shell/lyns-shell/issues/1438).
